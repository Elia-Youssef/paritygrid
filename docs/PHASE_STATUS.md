...
|---|---|---|
| P7.9 — concurrent result coordinator and serialized SQLite boundary | Accepted on the phase branch | `result_coordinator` implements the parent-side `ConcurrentResultCoordinator`: results arrive through the bounded result channel; pre-admission validation checks the contract version, run/node/partition/work identity, attempt, lease fence and owner, control generation, artifact references against registered admission facts, metrics, failure detail, and cleanup evidence — a rejected request submits zero writer commands and retains the lease for one corrected bounded resubmission. Writer admission is serialized under one lock and rebases the CURRENT run, node-aggregate, and event-frontier versions through the reader port, never trusting assignment-time row versions, so deliberately reversed completions commit correct rebased aggregates with contiguous per-run events. Only the durable commit acknowledgement unregisters the assignment, releases the dependency through the concurrent scheduler, frees the scheduled-work capacity, and emits one bounded telemetry record. Known rollbacks map to bounded persistence retry; an unknown outcome marks recovery required, closes producer flow, records the ambiguous identity, stops new admission, and never infers durability from the in-memory result. Stale, duplicate, forged, and mismatched results fail closed. Every SQLite writer and repository stays on the parent side behind the coordinator's ports, and source-scan regressions prove neither the module nor any runner double touches a database driver. 194 deterministic controlled-double regressions hold the module at 100% branch coverage with zero flakiness. |
| P7.8 — versioned concurrency telemetry schemas | Accepted on the phase branch | `telemetry` defines schema version 1 records for the eight observation kinds — queue depth, active capacity, capacity wait, service duration, blocked writer, dropped telemetry, cleanup state, and unresolved resources — with bounded metric names, labels, values, series counts, and a 4,096-byte wire bound, strict redaction of secret markers, credentials, and path shapes, canonical deterministic wire encodings whose strict inverse fails closed on unknown schemas, and helper constructors over P7.6 capacity and P7.7 channel facts. `TelemetryCollector` is a bounded drop-oldest sink that never blocks or grows and reports counted drops. Non-authority is proven structurally and behaviorally: the module imports no scheduler, persistence, lease, or checkpoint authority, records expose no mutating surface, and a regression holds a real `ConcurrentScheduler` frontier byte-identical across telemetry bursts, drops, and drains. 303 deterministic regressions hold the module at 100% branch coverage. |
...
| P7.3 — concurrent scheduler and SchedulerFrontierV2 | Accepted on the phase branch | `SchedulerFrontierV2` (schema version 2) durably records the plan fingerprint, control generation, per-node aggregates, deterministic ready work identities, admitted work with lease fences, results awaiting durable commit, retry waits with eligibility times, control state, and recovery-required reason with full cross-field fail-closed validation and exact serialization round trips. `ConcurrentScheduler` admits `(run_id, node_id, partition_key)` identities in deterministic plan-node-then-partition order across independent ready nodes and partitions, holds capacity through `mark_result_received` until `commit_result`, releases successor barriers only when every predecessor work item is durably succeeded, permanently blocks successors of blocked-terminal aggregates, moves retry waits through injected eligibility times without reading a clock, and enforces the running/quiescing/paused/cancelling/recovery-required control machine with generation bumps. Unknown frontier versions and malformed identity relationships fail closed. `tests/execution/test_concurrent_scheduler.py` holds 176 deterministic regressions at 100% module branch coverage. |
...
P7.10 — concurrent pause, cancellation, lifecycle, and durable recovery.
...
- P7.10 through P7.19 are not implemented.
- No threaded, asyncio, process, or interpreter strategy is accepted; the sequential runner remains the only execution strategy.
- Concurrent pause, cancellation, lifecycle, and durable recovery coordination (P7.10), the shared conformance suites (P7.11), and every actual strategy and pool (P7.12–P7.16, P7.19) do not exist.
- The runner-neutral contract, scheduler frontier, captured settings, clock/rate policy, capacity limiters, bounded channels, concurrency telemetry, and the parent-side result coordinator are accepted as standalone application-owned capabilities; no runner consumes them yet.
