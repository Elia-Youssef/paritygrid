...
export function Overview() {
  return (
    <div id="overview" className="mx-auto max-w-[var(--content-max)]">
      <section className="grid gap-10 border-b border-border pb-10 xl:grid-cols-overview-hero xl:items-end">
        <div className="max-w-4xl">
          <StatusBadge state="active">System foundation</StatusBadge>
          <h1
            data-page-title
            tabIndex={-1}
            className="mt-6 max-w-4xl rounded-sm text-hero leading-hero font-semibold tracking-hero text-balance focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-[var(--focus-ring)]"
          >
        ...
        <div className="border-l-2 border-active pl-5">
          <p className="font-mono text-xs tracking-eyebrow text-active uppercase">
            Design principle 01
          </p>
      ...
      <div className="grid gap-6 py-8 xl:grid-cols-overview-content">
        <section
          id="execution-path"
        ...
        >
          ...
          <footer className="grid gap-px border-t border-border bg-border sm:grid-cols-3">
            <div className="bg-surface-quiet p-4">
              <p className="font-mono text-2xs text-muted uppercase">Delivery</p>
              <p className="mt-1 text-sm font-medium">At-least-once execution</p>
            </div>
            <div className="bg-surface-quiet p-4">
              <p className="font-mono text-2xs text-muted uppercase">Effects</p>
              <p className="mt-1 text-sm font-medium">Idempotent and reviewed</p>
            </div>
            <div className="bg-surface-quiet p-4">
              <p className="font-mono text-2xs text-muted uppercase">Verification</p>
        ...
        >
          ...
          <dl className="mt-6 divide-y divide-border border-y border-border">
            {readinessItems.map(({ detail, label, state }) => (
              ...
              >
                <dt className="col-start-1 row-start-1 text-sm font-medium">{label}</dt>
                <dd className="col-start-1 row-start-2 font-mono text-2xs text-muted">
                  {detail}
                </dd>
