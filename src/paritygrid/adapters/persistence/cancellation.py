"""Short-transaction SQLite reader for bounded cancellation coordination."""
...
from paritygrid.application.execution.cancellation import CancellationDurableState
...
class SQLiteCancellationStateReader:
    ...
    def __init__(self, database: SQLiteDatabase) -> None:
        if type(database) is not SQLiteDatabase:
            raise TypeError("cancellation state reader database must use SQLiteDatabase")
    ...
    def read(self, run_id: RunId, /) -> CancellationDurableState:
        ...
        if type(run_id) is not RunId:
            raise TypeError("cancellation state reader run identity must use RunId")
        ...
        if run is None or counter is None:
            raise ExecutionRecordNotFoundError("cancellation durable run frontier does not exist")
...
__all__ = ["SQLiteCancellationStateReader"]
